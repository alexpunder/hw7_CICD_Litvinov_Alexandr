...
Так как регистрация нового профиля в `GitLab` из России недоступна, было разрешено выполнить задание в `GitVerse`. Однако, в последнем используется синтаксис `GitHub`, поэтому их `workflow` одинаковые.
...
- Если мы понимаем, что необходимо полностью вернуть предыдущую модель, то происходит мгновенный откат: меняем 90% `blue` на 100% в `split_clients`, перезапускаем `Nginx`, и весь трафик возвращается на Blue
...
Написать полноценный `workflow` для GitLab не получилось, а предложенная альтернатива в виде GitVers использует обычный синтаксис `GitHub`, который отличается. При составлении пайплайна самое сложное было находить нужные `action/...` под те или иные действия, будь то регистрация или отправка образа.  

Стратегии развертывания ясны, понятны причины их появления и необходимость применения. В "песочнице" задания это выглядит довольно просто, однако в реальной среде скорее всего потребуются уже другой уровень компетенций.
